...
export async function getAggregatedAnalytics(
  ...
  try {
    ...
    const zoneAnalytics: ZoneAnalytics[] = Array.from(zoneMap.entries())
      .map(([zoneId, data]) => {
 ...
 */
export async function getTrendAnalytics(days: number = 30): Promise<TrendData[]> {
  try {
...
export async function getPeakAnalysis(
  ...
  try {
    ...
    const quietHourData = hourlyArray.reduce(
...
export async function getUserStats(userId: string): Promise<UserStats | null> {
  try {
    ...
    const favoriteCategory =
      sortedCategories.length > 0 && sortedCategories[0] ? sortedCategories[0][0] : 'N/A';
 ...
 */
export async function getZonePopularTimes(
  zoneId: string
