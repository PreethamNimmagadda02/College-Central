...
const COLOR_OPTIONS = [
  { name: 'Green (A+)', value: 'text-green-600 bg-green-100 dark:bg-green-900/30 dark:text-green-400' },
  { name: 'Emerald (A)', value: 'text-emerald-800 bg-emerald-200 dark:bg-emerald-900/40 dark:text-emerald-200' },
  ...
  { name: 'Amber (C+)', value: 'text-amber-700 bg-amber-200 dark:bg-amber-900/40 dark:text-amber-300' },
  { name: 'Yellow (C)', value: 'text-yellow-600 bg-yellow-100 dark:bg-yellow-900/30 dark:text-yellow-400' },
  { name: 'Orange (D)', value: 'text-orange-600 bg-orange-100 dark:bg-orange-900/30 dark:text-orange-400' },
  { name: 'Red (F)', value: 'text-red-600 bg-red-100 dark:bg-red-900/30 dark:text-red-400' },
  { name: 'Purple', value: 'text-purple-600 bg-purple-100 dark:bg-purple-900/30 dark:text-purple-400' },
  { name: 'Pink', value: 'text-pink-600 bg-pink-100 dark:bg-pink-900/30 dark:text-pink-400' },
  { name: 'Indigo', value: 'text-indigo-600 bg-indigo-100 dark:bg-indigo-900/30 dark:text-indigo-400' },
...
const GradingEditor: React.FC<GradingEditorProps> = ({
  ...
  const gradingScale = config.gradingScale && config.gradingScale.length > 0 
    ? config.gradingScale 
  ...
  return (
    <AdminPageLayout>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold admin-gradient-text">Grading Scale Configuration</h2>
          <p className="text-slate-600 dark:text-slate-400 text-sm mt-1">
            Configure letter grades and their corresponding grade points used for SGPA/CGPA calculations
          </p>
        </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="text-sm text-amber-800 dark:text-amber-200">
              ...
              <p className="text-amber-700 dark:text-amber-300 mt-1">
                These are the standard grades. Click "Save Defaults" to save them to the database, or customize as needed.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setNewGrade({ ...newGrade, points: parseFloat(e.target.value) || 0 })}
                placeholder="e.g., 10"
                className="admin-input w-full"
                min="0"
                max="15"
                  ...
                  <tr key={grade.id} className="hover:bg-indigo-500/10 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      {editingId === grade.id ? (
                        <input
                          type="text"
                          value={grade.grade}
                          onChange={(e) => updateGrade(grade.id, { grade: e.target.value.toUpperCase() })}
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button
                          onClick={() => {
                            if (window.confirm(`Delete grade "${grade.grade}"? This action cannot be undone.`)) {
        ...
        <div className="space-y-3">
          {gradingScale.length === 0 ? (
            ...
            gradingScale.map((grade, index) => (
              <div
                key={grade.id}
                className="admin-list-item flex flex-col gap-3 p-4"
              >
