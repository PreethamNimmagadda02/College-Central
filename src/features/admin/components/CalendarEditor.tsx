...
interface Props {
  ...
  updateCalendarDates?: (dates: { semesterStartDate?: string; semesterEndDate?: string; semesterName?: string }) => void;
}
    ...
    <AdminPageLayout>
      ...
      <div className="admin-card">
        ...
        <p className="text-indigo-300 text-sm mb-4">
          Set the name of the current/ongoing semester. This will be displayed in the user dashboard's semester progress section.
        </p>
          ...
          {editingSemesterName ? (
            ...
            <div className="flex items-center justify-between">
              <p className="text-white text-lg font-medium">
                {config.calendar.semesterName || <span className="text-slate-400 italic">Auto-detect from events</span>}
              </p>
              <button
                onClick={() => {
                  setTempSemesterName(config.calendar.semesterName || '');
      ...
      {(() => {
        ...
        const detectedEnd = endEvents[endEvents.length - 1]?.endDate || endEvents[endEvents.length - 1]?.date;
        ...
        return (
          <div className="admin-card">
            ...
            <p className="text-indigo-300 text-sm mb-4">
              These dates are automatically detected from "Start of Semester" and "End-Semester Exams" events.
            </p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="p-4 rounded-xl bg-slate-700/50 border border-slate-600/50">
                <label className="text-indigo-400 text-xs font-medium mb-2 block">Year Start Date</label>
                <p className="text-white text-lg font-medium">
                  {detectedStart ? formatDate(detectedStart) : <span className="text-slate-400 italic">No "Start of Semester" event found</span>}
                </p>
              </div>
              <div className="p-4 rounded-xl bg-slate-700/50 border border-slate-600/50">
                <label className="text-indigo-400 text-xs font-medium mb-2 block">Year End Date</label>
                <p className="text-white text-lg font-medium">
                  {detectedEnd ? formatDate(detectedEnd) : <span className="text-slate-400 italic">No "End-Semester Exams" event found</span>}
                </p>
              </div>
            </div>
            {(!detectedStart || !detectedEnd) && (
              <div className="mt-4 p-3 rounded-lg bg-amber-500/10 border border-amber-500/30">
                <p className="text-amber-400 text-sm">
                  💡 Add events with type "Start of Semester" and "End-Semester Exams" for automatic detection.
                </p>
              </div>
            )}
          </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  value={newEvent.type}
                  onChange={e => setNewEvent({ ...newEvent, type: e.target.value as AdminCalendarEvent['type'] })}
                >
                  {EVENT_TYPES.map(type => (
                    <option key={type} value={type}>{type}</option>
                  ))}
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={e => setEditingEvent({ ...editingEvent, type: e.target.value as AdminCalendarEvent['type'] })}
                >
                  {EVENT_TYPES.map(type => (
                    <option key={type} value={type}>{type}</option>
                  ))}
              ...
              <div className="flex gap-3 mt-6">
                ...
                <button onClick={() => setEditingEvent(null)} className="admin-btn admin-btn-secondary flex-1">
                  Cancel
                </button>
              </div>
            </div>
