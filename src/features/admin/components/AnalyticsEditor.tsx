...
const AnalyticsEditor: React.FC = () => {
  ...
  const [selectedFilter, setSelectedFilter] = useState<{ title: string; users: UserData[] } | null>(null);
  ...
  const fetchAnalytics = async () => {
    ...
    try {
      ...
      const profileComplete = users.filter(u => u.profilePicture && u.bio && u.bio.trim().length > 0).length;
      const profileCompletionRate = users.length > 0 ? Math.round((profileComplete / users.length) * 100) : 0;
  ...
  const filterBySocialLinks = () => {
    if (!stats) return;
    const filtered = stats.allUsers.filter(u => u.socialLinks && Object.keys(u.socialLinks).length > 0);
  ...
  const filterByBranch = (branchName: string) => {
    ...
    if (branchName === 'Other') {
      ...
      const filtered = stats.allUsers.filter(u => 
        (u.branch || '').trim().toLowerCase() === normalizedSearch
      );
    ...
    <AdminPageLayout>
              ...
              <ResponsiveContainer width="100%" height="100%" debounce={50}>
                <BarChart 
                  data={(() => {
                    ...
                    const otherCount = stats.branchDistribution.slice(5).reduce((sum, b) => sum + b.count, 0);
                    if (otherCount > 0) {
          ...
          {stats.branchDistribution.length > 0 ? (
            <div style={{ width: '100%', height: 280 }}>
              <ResponsiveContainer width="100%" height="100%" debounce={50}>
                ...
                >
                  <XAxis type="number" stroke="#94a3b8" />
                  <YAxis type="category" dataKey="name" stroke="#94a3b8" width={80} tick={{ fontSize: 11 }} />
                  <Tooltip 
                    contentStyle={{ 
                      backgroundColor: '#1e293b', 
                      border: '1px solid rgba(96, 165, 250, 0.3)',
      ...
      {selectedFilter && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={closeModal}>
          <div className="admin-card max-w-2xl w-full max-h-[80vh] overflow-hidden" onClick={e => e.stopPropagation()}>
      ...
      {userToDelete && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => !deleting && setUserToDelete(null)}>
          <div className="admin-card max-w-md w-full" onClick={e => e.stopPropagation()}>
            <div className="text-center">
              ...
              <p className="text-indigo-300 mb-6">
                Are you sure you want to remove <span className="font-semibold text-white">{userToDelete.name}</span>?
                <br />
