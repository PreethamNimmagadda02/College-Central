...
const CGPAForecaster: React.FC = React.memo(() => {
  ...
  // Use user.gradeScenarios from Firestore (via UserContext)
  // Initialize from user object, default to empty array
  const savedScenarios = useMemo(() => user?.gradeScenarios || [], [user?.gradeScenarios]);
  ...
  const saveCurrentScenario = async () => {
    ...
    if (savedScenarios.length >= 3) return; // Max 3 scenarios
    if (!user) return;
    ...
    const newScenario: SavedScenario = {
      ...
      projectedSgpa: calculations.projectedSgpa,
      projectedCgpa: calculations.projectedCgpa,
      savedAt: new Date().toLocaleDateString(),
    };

    try {
      const updatedScenarios = [...savedScenarios, newScenario];
      await updateUser({ gradeScenarios: updatedScenarios });
  ...
  const loadScenario = (scenario: SavedScenario) => {
    setProjectedGrades(scenario.grades);
  ...
  return (
    ...
    >
              ...
              >
                ...
                <div className="relative z-10 flex items-center gap-2 sm:gap-3">
                  <select
                    ...
                    className={`px-3 sm:px-4 py-1.5 sm:py-2 font-semibold rounded-lg border-2 focus:outline-none focus:ring-2 transition-all hover:scale-105 text-sm sm:text-base cursor-pointer text-center ${getGradeColor(projectedGrades[course.courseCode] || 'A')} ${getGradeOutlineStyle(projectedGrades[course.courseCode] || 'A')}`}
                  >
      ...
      <div className="bg-white dark:bg-dark-card rounded-xl shadow-lg overflow-hidden">
        <div className="p-4 sm:p-6 border-b border-slate-200 dark:border-slate-700 bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-900/10 dark:to-orange-900/10">
          <div className="flex items-center justify-between">
          ...
          {savedScenarios.length === 0 ? (
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                {selectedScenarios.length > 0 && (
                  <span className="text-xs font-medium px-2 py-1 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 rounded-full">
                    {selectedScenarios.length}/3 selected
                  </span>
                )}
              </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {selectedScenarios.map((id, index) => {
                    ...
                    return (
                      <div
                        key={id}
                        className={`relative overflow-hidden bg-gradient-to-br ${getScenarioColor(index)} text-white p-5 rounded-xl shadow-lg`}
                      >
                        <div className="absolute top-3 right-3 w-7 h-7 rounded-full bg-white/20 flex items-center justify-center text-sm font-bold">
                          {index + 1}
                        </div>
                        <h4 className="font-bold text-lg mb-3 pr-8">{scenario.name}</h4>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {savedScenarios.map((scenario) => (
                <div
                  key={scenario.id}
                  className="group relative bg-slate-50 dark:bg-slate-800/50 rounded-xl p-4 hover:shadow-lg transition-all border border-slate-200 dark:border-slate-700 hover:border-amber-300 dark:hover:border-amber-700"
                >
                  ...
                  <button
                    ...
                    className="w-full py-2.5 text-sm font-semibold rounded-lg bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white shadow-sm hover:shadow-md transition-all"
                  >
                    Load Scenario
                  </button>
                </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-dark-card rounded-xl shadow-2xl max-w-md w-full p-6 animate-slideIn">
            ...
            <input
              ...
              className="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary dark:bg-slate-700 dark:text-white mb-4"
              autoFocus
              onKeyDown={(e) => {
            ...
            <div className="flex justify-end gap-2">
              ...
              <button
                ...
                className="px-4 py-2 text-sm font-medium bg-amber-500 hover:bg-amber-600 disabled:bg-slate-300 disabled:cursor-not-allowed text-white rounded-lg transition-colors"
              >
                Save Scenario
              </button>
            </div>
...
const Grades: React.FC = () => {
  ...
  const [showImprovementForecaster, setShowImprovementForecaster] = useState(false);
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-4 sm:space-y-6 p-3 sm:p-4 lg:p-6">
      ...
      <div className="flex flex-wrap gap-2 sm:gap-3">
        ...
        <button
          onClick={() => {
            setShowImprovementForecaster(!showImprovementForecaster);
            if (showForecaster) setShowForecaster(false);
            if (showAnalytics) setShowAnalytics(false);
          }}
