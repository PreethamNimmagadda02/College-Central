...
### 3.1 Project Structure (Flat File Architecture)

```
College Central/
│
├── index.tsx                    # Application entry point
├── App.tsx                      # Router configuration & provider nesting
├── firebaseConfig.ts            # Firebase initialization (Compat API)
...
│   ├── AcademicCalendar.tsx    # Academic events calendar
│   ├── CollegeForms.tsx        # College forms repository
│   ├── Directory.tsx           # Campus directory
...
│   ├── GradesContext.tsx       # Academic grades state
│   ├── ScheduleContext.tsx     # Class schedule state
│   ├── CalendarContext.tsx     # Calendar events state
│   ├── CampusMapContext.tsx    # Campus locations state
│   └── FormsContext.tsx        # College forms state
│
...
│   └── activityService.ts      # User activity logging
│
...
- **Simplicity**: All source files in root, easy to navigate
- **No src/ directory**: Direct access to code files
...
```typescript
// lazyWithRetry.ts - Custom lazy loading with retry logic
